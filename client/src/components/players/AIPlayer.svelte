<script lang="ts">
  ...
  function playRandomCard() {
    if (cards.length === 0 || !isActive || passingPhase) return;
  ...
  function selectCardsForPassing() {
    if (!passingPhase || selectedCardsForPassing.length >= 3) return;
  ...
  // Auto-play when it becomes this AI's turn
  $: if (isActive && !passingPhase && !isThinking) {
    playRandomCard();
  }
  
  // Auto-select cards for passing when entering passing phase
  $: if (passingPhase && selectedCardsForPassing.length === 0) {
